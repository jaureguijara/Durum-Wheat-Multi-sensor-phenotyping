...
def correlate_indices_with_targets(df, index_columns, dataset_name, yield_col='Yield (kg/ha)'):
    df_clean = df.dropna(subset=index_columns + [yield_col])
    
    results = []
    for col in index_columns:
        corr_yield = df_clean[col].corr(df_clean[yield_col], method='pearson')
        results.append({
            'Dataset': dataset_name,
            'Index': col,
            'Correlation_with_Yield': corr_yield
        })
    
    corr_df = pd.DataFrame(results).sort_values('Correlation_with_Yield', ascending=False)
    return corr_df

corr_an   = correlate_indices_with_targets(df_an,   vi_all, "Anthesis")
corr_gf   = correlate_indices_with_targets(df_gf,   vi_all, "Grain Filling")
corr_diff = correlate_indices_with_targets(df_diff, vi_all, "Index Difference")

corr_summary = pd.concat([corr_an, corr_gf, corr_diff], ignore_index=True)

corr_summary = corr_summary.sort_values(['Dataset', 'Correlation_with_Yield'], ascending=[True, False])

print(corr_summary.tail(20))

os.chdir('..')
os.chdir('..\\results')

corr_summary.to_csv("VI_yield_correlations_all.csv", index=False)
