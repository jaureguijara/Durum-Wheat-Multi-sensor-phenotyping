...
df['Selection observed'] = np.where(
    (df['Yield group'] == 'Low yield') | (df['Stability group'] == 'Low stability'),
    'Discard',
    'Keep'
)
...
# List reordered 
indices_all = ['NDVI_UAV', 'RDVI_UAV', 'SAVI_UAV', 'TCARI_UAV', 
               'NDVI_ground', 'GA_UAV', 'u*_UAV', 'v*_UAV',
               'Hue_UAV',  'GGA_ground','u*_ground', 'v*_ground',
               'Hue_ground',  'CSI_UAV','a*_ground', 'b*_ground', 
              'Lightness_UAV', 'CSI_ground', 'NGRDIveg_UAV',  'TGI_UAV',
              'Intensity_ground', 'Tveg_mean_UAV', 'NGRDIveg_ground', 'TGI_ground'] 
...
# Run for All indices (using Selection observed)
...
def plot_selection_groups_all(df, indices, valladolid_results, aranjuez_results):
    ...
    fig, axes = plt.subplots(nrows=nrows, ncols=ncols, figsize=(20, 24), sharex=True, constrained_layout=True)
    axes = axes.flatten()
    ...
    for i, index in enumerate(indices):
        ...
        if origin_labels:
            ax.text(
                ...
                ha='left', va='top',
                fontsize=13, fontstyle='italic', color='dimgray'
            )
    ...
    fig.text(0.5, -0.02, 'Growing Degree Days', ha='center', fontsize=24)
    fig.text(-0.015, 0.5, 'Normalized percentage difference to the mean', va='center', rotation='vertical', fontsize=24)

    axes[3].legend(title='Selection', loc='upper center', bbox_to_anchor=(0.6, 1.6), frameon=True,
                   ncol=2, fontsize=20, title_fontsize=20, borderpad=1.25, edgecolor='black')

    fig.suptitle(f"Seasonal reflectance phenotypes for genotype selection", fontsize=26, y=0.98)
