...
def calculate_index_stability(df, veg_index_columns):
    ...
    for index in veg_index_columns:
        ...
        df['Environmental_Index'] = (
            df.groupby('Loc_Treatment', observed=False)[index].transform('mean')
        )

        # Fit Linear Model for Stability
        ...
        # Main environmental effect (slope)
        ...
        # Combine stability and intercepts
        stability_df = pd.merge(interaction_df[['Code', 'I_Stability']], intercept_df, on='Code')

        # Add vegetation index information
        stability_df['Index'] = index
        
        # Clean up Code column
...
stability_diff, wide_diff = calculate_index_stability(df_diff_n, vi_all)
stability_an, wide_an = calculate_index_stability(df_an_n, vi_all)
stability_gf, wide_gf = calculate_index_stability(df_gf_n, vi_all)
...
VI_selected_an = ['CSI_UAV', 'NGRDIveg_UAV', 'RDVI_UAV', 'TCARI_UAV', 'NDVI_ground', 'b*_ground']
VI_selected_gf = ['v*_UAV', 'TCARI_UAV', 'NDVI_ground', 'Intensity_ground', 'u*_ground', 'GGA_ground']
VI_selected_diff = ['TGI_UAV', 'RDVI_UAV', 'NDVI_ground', 'a*_ground', 'NGRDIveg_ground', 'TGI_ground']
...
for ax, VI_selected in zip(axes, vi_lists):
    ...
    ax.text(0.5, -0.20, vi_text_2, ha='center', va='top',
            fontsize=16, fontweight='bold', transform=ax.transAxes)
