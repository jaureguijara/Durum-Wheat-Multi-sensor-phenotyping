...
yield_groups = yield_groups.rename(columns={'Yield_group': 'Yield group'})

yield_groups = pd.merge(yield_groups, stability_df[['Code', 'Stability', 'Stability group']],
                        on=['Code'], how ='left')

df = pd.merge(df, yield_groups[['Code', 'Yield group', 'emmean', 'Stability group', 'Stability']], 
               on=['Code'], how='left')
...
yield_indices_an = ['NDVI_UAV', 'SAVI_UAV','RDVI_UAV','GA_UAV', 'Hue_UAV', 'Hue_ground']
yield_indices_gf = ['NDVI_ground',  'a*_ground','v*_UAV','v*_ground', 'Tveg_mean_UAV',  'CSI_ground']
yield_indices_diff = ['NGRDIveg_ground','Lightness_UAV', 'u*_UAV', 'v*_UAV', 'GA_UAV', 'Tveg_mean_UAV']

stability_indices_an = ['NDVI_UAV', 'SAVI_UAV', 'GGA_UAV', 'NGRDI_UAV', 'u*_ground', 'Tveg_mean_UAV']
stability_indices_gf = ['NDVI_ground',  'Hue_UAV', 'v*_ground', 'Saturation_UAV',  'Tveg_mean_UAV']
stability_indices_diff = [ 'NGRDIveg_UAV', 'Lightness_ground','Saturation_UAV', 'a*_ground', 'TGI_ground','NDVI_UAV']
...
def plot_ideotype(df, yield_indices, stability_indices, dataset_name, treatment=None, 
    ...
    max_rows = max(len(yield_indices), len(stability_indices))

    for i in range(max_rows):
        # === YIELD SIDE ===
        if i < len(yield_indices):
            index = yield_indices[i]
            ax = axes[i, 0]
            plot_data = df.copy()
            ...
            plot_data_grouped = (
                plot_data.groupby(['Yield group', 'GDD', 'Loc'])[index]
                .agg(mean='mean', se=lambda x: np.std(x, ddof=1) / np.sqrt(len(x)))
                .reset_index()
            )

            sns.lineplot(
                data=plot_data_grouped, x='GDD', y='mean', hue='Yield group',
                marker='o', palette=yield_palette, hue_order=yield_hue_order, err_style=None, ax=ax
            )

            for j, group in enumerate(yield_hue_order):
                group_data = plot_data_grouped[plot_data_grouped['Yield group'] == group]
                ax.errorbar(group_data['GDD'], group_data['mean'], yerr=group_data['se'], fmt='none',
                            ecolor=yield_palette[j], alpha=0.7, capsize=3)
            ...
            for location, results_dict, label_color, xy_offset in [
                ('Valladolid', valladolid_yield_results, '#D32F2F', (-20, 10)),
                ('Aranjuez', aranjuez_yield_results, '#9B59B6', (-20, 35))
            ]:
                if results_dict and index in results_dict:
                    for result_dict in results_dict[index]:
                        gdd_value = result_dict['GDD']
                        ...
                        if not gdd_data.empty:
                            x_pos = gdd_data['GDD'].values[0]
                            y_pos = gdd_data['mean'].mean()
                            ax.annotate("\n".join([g.strip() for g in significant_groups.split(",")]),
                                        (x_pos, y_pos), textcoords="offset points",
                                        xytext=xy_offset, fontsize=12, color=label_color)

            if yield_handles is None:
                yield_handles, yield_labels = ax.get_legend_handles_labels()

        # === STABILITY SIDE ===
        if i < len(stability_indices):
            index = stability_indices[i]
            ax = axes[i, 1]
            plot_data = df.copy()
            ...
            plot_data_grouped = (
                plot_data.groupby(['Stability group', 'GDD', 'Loc'])[index]
                .agg(mean='mean', se=lambda x: np.std(x, ddof=1) / np.sqrt(len(x)))
                .reset_index()
            )

            sns.lineplot(
                data=plot_data_grouped, x='GDD', y='mean', hue='Stability group',
                marker='o', palette=stability_palette, hue_order=stability_hue_order, err_style=None, ax=ax
            )

            for j, group in enumerate(stability_hue_order):
                group_data = plot_data_grouped[plot_data_grouped['Stability group'] == group]
                ax.errorbar(group_data['GDD'], group_data['mean'], yerr=group_data['se'], fmt='none',
                            ecolor=stability_palette[j], alpha=0.7, capsize=3)
            ...
            for location, results_dict, label_color, xy_offset in [
                ('Valladolid', valladolid_stability_results, '#D32F2F', (-20, 10)),
                ('Aranjuez', aranjuez_stability_results, '#9B59B6', (-20, 35))
            ]:
                if results_dict and index in results_dict:
                    for result_dict in results_dict[index]:
                        gdd_value = result_dict['GDD']
            ...
            if stability_handles is None:
                stability_handles, stability_labels = ax.get_legend_handles_labels()

    # === layout tweaks ===
    try:
        fig.delaxes(axes[5, 1])  # delete empty stability axis
    except Exception:
        pass

    last_stab_row = len(stability_indices) - 1
    if 0 <= last_stab_row <= 5:
        axes[last_stab_row, 1].tick_params(axis='x', labelbottom=True)
    ...
    fig.text(0.5, -0.01, 'Growing Degree Days', ha='center', fontsize=16)
    fig.text(-0.01, 0.5, 'Normalized percentage difference to the mean', va='center', rotation='vertical', fontsize=16)
    fig.suptitle(f"Seasonal reflectance phenotypes for yield and yield stability \n (features selected at {dataset_name})",
                 fontsize=18, y=1.03)
